...
func ERC20Detection(ctx context.Context, client *ethclient.Client, contractAddress common.Address) (*Erc20Tokens, error) {
	...
	if err != nil {
		...
		if hexcode := hex.EncodeToString(bytecode); strings.Contains(hexcode, "a9059cbb") && strings.Contains(hexcode, "18160ddd") {
			...
			name, err := ERC20_Name(ctx, client, *erc20ABI, address)
			if err != nil {
				erc20Token.Value.Type = TokenTypeError
			} else {
			...
			if erc20Token.Value.Type == TokenTypeERC20 {
				symbol, err := ERC20_Symbol(ctx, client, *erc20ABI, address)
				if err != nil {
					erc20Token.Value.Type = TokenTypeError
				} else {
			...
			if erc20Token.Value.Type == TokenTypeERC20 {
				decimals, err := ERC20_Decimals(ctx, client, *erc20ABI, address)
				if err != nil {
					erc20Token.Value.Type = TokenTypeError
				} else {
	...
	if erc20Token.Value.Type != TokenTypeERC20 {
		...
		if result != nil {
			// it is a proxy
			targetAddress := MixedcaseAddress(result.Target)
