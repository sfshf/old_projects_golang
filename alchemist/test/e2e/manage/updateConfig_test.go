...
func TestUpdateConfig(t *testing.T) {
	// mock data
	testAppID := "alchemist-e2e-TestUpdateConfig"
	testConfig := `{
	"appID":"alchemist-e2e-TestUpdateConfig",
	...
	newAppConfig := AppConfig{
		...
		Config: `{
	"appID":"alchemist-e2e-TestUpdateConfig",
	"productId":"alchemist-e2e-update",
	"deviceCheck": {
		"keyID": "deviceCheckKeyID",
	"issuerID": "deviceCheckIssuerID",
	"privKeyPem": "deviceCheckPrivKeyPem"
	...
	// send request
	resp, err := postJsonRequest(_kongDNS+"/alchemist/console/updateConfig/v1", &reqData, nil, &respData, nil)
	if err != nil {
		t.Error(err)
		return
	}
	...
	if err := _alchemistGormDB.Table(TableNameAppConfig).
		Where(`app = ? AND deleted_at = 0`, testAppID).
...
func TestUpdateConfig_EmptyParameter(t *testing.T) {
	reqData := struct {
		Password string `json:"password"`
		ID       int64  `json:"id"`
		Config   string `json:"config"`
