...
import (
	...
	grpcserver "github.com/nextsurfer/tester/internal/app/grpc/server"
...
func NewApplication(ctx context.Context, name string, port int, host string, appEnv int, redisDNS, mysqlDNS string, tomlPath, mongodbUri string) (*Application, error) {
	var err error
	app := &Application{
		Name: name,
		Port: port,
...
func (app *Application) RegisterServers(ctx context.Context) error {
	// register servers
	testerServer, err := grpcserver.NewTesterServer(
		ctx,
		app.Name,
		app.Env,
		app.Logger,
	...
	tester_api.RegisterTesterServiceServer(
		app.Server.GrpcServer(),
		testerServer)
	app.Logger.Info("RegisterTesterServiceServer success")
	return nil
}
