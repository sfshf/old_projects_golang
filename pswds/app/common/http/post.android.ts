...
export const post = async (
  ...
  // 1. pre handles
  let lSessionID = '';
  const slarkInfo = currentSlarkInfo();
  if (slarkInfo) {
    lSessionID = slarkInfo.lSessionID;
  }
  ...
  try {
    ...
    // 3-3. handle response headers
    for (const [name, value] of resp.headers) {
      if (
        name === 'set-cookie' &&
        value.indexOf(DefaultCookieSessionKey) !== -1
      ) {
        lSessionID = value;
        break;
      }
