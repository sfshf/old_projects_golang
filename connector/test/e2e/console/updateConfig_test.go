...
func TestUpdateConfig(t *testing.T) {
	// mock data
	testApp := "connector-e2e-app-TestUpdateConfig"
	testConfig := `{
	"appName": "connector-e2e-app-TestUpdateConfig-update"
}`

	newAppConfig := AppConfig{
		App: testApp,
		Config: `{
	"appName": "connector-e2e-app-TestUpdateConfig"
}`,
	}
	...
	// send request
	resp, err := postJsonRequest(_kongDNS+"/riki/console/updateConfig/v1", &reqData, nil, &respData, nil)
	if err != nil {
		t.Error(err)
		return
	}
...
func TestUpdateConfig_EmptyApiKey(t *testing.T) {
	reqData := struct {
		ApiKey string `json:"apiKey"`
		App    string `json:"app"`
		Config string `json:"config"`
