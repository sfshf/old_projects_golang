...
const columns: readonly Column[] = [
  ...
  { id: 'publicKey', label: 'PublicKey', minWidth: 100 },
...
export default function Home() {
  ...
  const listPrivateKey = async () => {
    const reqData = {
      apiKey: shared.getPassword(),
      app: app,
    };
    post('/riki/console/listPrivateKey/v1', reqData, (respData) => {
      setList(respData.data.list);
    });
  };

  const addPrivateKey = async (app: string, keyID: string) => {
    const reqData = {
      apiKey: shared.getPassword(),
      app: app,
      keyID: keyID,
    };
    post('/riki/console/addPrivateKey/v1', reqData);
  };

  const removePrivateKey = async (app: string, keyID: string) => {
    const reqData = {
      apiKey: shared.getPassword(),
      app: app,
      keyID: keyID,
    };
    post('/riki/console/removePrivateKey/v1', reqData);
  };
        ...
        <DialogActions>
          ...
          {!isShow && (
            <Button
              onClick={() => {
                if (isDelete) {
                  removePrivateKey(row.app, row.keyID);
                } else {
                  if (isCreate) {
                    addPrivateKey(dialogApp, dialogKeyID);
                  } else {
                  }
                }
              }}
