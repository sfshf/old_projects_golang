...
var (
	...
	_pswdsMysqlDsn = os.Getenv("PSWDS_MYSQL_DNS") + "?charset=utf8&parseTime=true"
	_pswdsGormDB   *gorm.DB

	// test account 1
	_testEmail       = "pswds-01@e2e-test.com"
	_testPassword    = "qwer1234"
	_testAccount     SlkUser
	_testApplication = "pswds-e2e"
	_testDeviceID    = "NOID"
	_testSession     SlkSession
	_testCookie      *http.Cookie

	// test account 2
	_testEmail2       = "pswds-02@e2e-test.com"
	_testPassword2    = "qwer1234"
	_testAccount2     SlkUser
	_testApplication2 = "pswds-e2e"
	_testDeviceID2    = "NOID"
...
func TestMain(m *testing.M) {
	...
	_pswdsGormDB, err = gorm.Open(mysql.Open(_pswdsMysqlDsn), &gorm.Config{})
	if err != nil {
		log.Fatalln(err)
	}
	...
	if err := _slarkGormDB.Table(TableNameSlkUser).
		...
		if err != gorm.ErrRecordNotFound {
			...
			_testAccount.Nickname = "pswds-e2e-01"
	...
	_, err = h2.Write([]byte(_testPassword2))
	if err != nil {
		log.Fatalln(err)
	}
	testPasswordHash2 := hex.EncodeToString(h2.Sum(nil))
	if err := _slarkGormDB.Table(TableNameSlkUser).
		Where(`email = ? AND password_hash = ? AND deleted_at = 0`,
			_testEmail2, testPasswordHash2).
		First(&_testAccount2).Error; err != nil {
		if err != gorm.ErrRecordNotFound {
			log.Fatalln(err)
		} else {
			_testAccount2.Email = _testEmail2
			_testAccount2.Nickname = "pswds-e2e-02"
			_testAccount2.PasswordHash = testPasswordHash2
			if err := _slarkGormDB.Table(TableNameSlkUser).
	...
	if err := _slarkGormDB.Table(TableNameSlkSession).
		Where(`user_id = ? AND application = ? AND device_id = ? AND deleted_at = 0`,
			_testAccount2.ID, _testApplication2, _testDeviceID2).
		...
		if err != gorm.ErrRecordNotFound {
			...
			_testSession2.Application = _testApplication2
			_testSession2.DeviceID = _testDeviceID2
