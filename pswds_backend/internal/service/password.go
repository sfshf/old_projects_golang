...
func NewPasswordService(ctx context.Context, pswdsService *PswdsService) *PasswordService {
	return &PasswordService{
		PswdsService: pswdsService,
	}
}

func (s *PasswordService) CreatePasswordRecord(ctx context.Context, rpcCtx *rpc.Context, req *pswds_api.CreatePasswordRecordRequest) *gerror.AppError {
	logger := s.Logger
	if rpcCtx != nil {
	...
	if record != nil {
		err = errors.New("password record exists")
...
func (s *PasswordService) validatePasswordRecord(ctx context.Context, rpcCtx *rpc.Context, userID int64, dataID string) (*PasswordRecord, *gerror.AppError) {
	logger := s.Logger
	if rpcCtx != nil {
		logger = rpcCtx.Logger
	}
	record, err := s.DaoManager.PasswordRecordDAO.GetByUserIDAndDataID(ctx, userID, dataID)
	if err != nil {
		logger.Error("internal error", zap.NamedError("appError", err))
	...
	if record == nil {
		err = errors.New("empty password record")
		logger.Error("bad request", zap.NamedError("appError", err))
...
func (s *PasswordService) UpdatePasswordRecord(ctx context.Context, rpcCtx *rpc.Context, req *pswds_api.UpdatePasswordRecordRequest) *gerror.AppError {
	logger := s.Logger
	if rpcCtx != nil {
		logger = rpcCtx.Logger
	}
	...
	record, appError := s.validatePasswordRecord(ctx, rpcCtx, loginInfo.UserID, req.DataID)
	if appError != nil {
		return appError
	}
	...
	if err := s.DaoManager.TransFunc(func(tx *gorm.DB) error {
		...
		// 2. password record
		if err := daoManager.PasswordRecordDAO.UpdateByUserIDAndDataID(ctx, loginInfo.UserID, req.DataID, &PasswordRecord{
			UpdatedAt: req.UpdatedAt,
			Content:   req.Content,
		}); err != nil {
			return err
...
func (s *PasswordService) DeletePasswordRecord(ctx context.Context, rpcCtx *rpc.Context, req *pswds_api.DeletePasswordRecordRequest) *gerror.AppError {
	logger := s.Logger
	if rpcCtx != nil {
		logger = rpcCtx.Logger
	}
	...
	if _, appError = s.validatePasswordRecord(ctx, rpcCtx, loginInfo.UserID, req.DataID); appError != nil {
		return appError
	}
	backup, appError := s.validateBackupRecord(ctx, rpcCtx, loginInfo.UserID)
	if appError != nil {
	...
	if err := s.DaoManager.TransFunc(func(tx *gorm.DB) error {
		...
		// 2. password record
		if err := daoManager.PasswordRecordDAO.DeleteByUserIDAndDataID(ctx, loginInfo.UserID, req.DataID); err != nil {
			return err
		}
		// 3. shared data
