...
func TestLogOutBySession(t *testing.T) {
	ctx := context.Background()
	testEmail := "Test@LogOutBySession.com"
	testNickname := "TestLogOutBySession"
	...
	// send request
	resp, err := postJsonRequest(_kongDNS+"/slark/user/logout/v1", nil, testCookie, &respData, nil)
	if err != nil {
		t.Error(err)
		return
	}
